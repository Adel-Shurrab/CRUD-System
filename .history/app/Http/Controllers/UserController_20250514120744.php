...
class UserController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if(!preg_match('/^[a-zA-Z\s]+$/', $validatedData['name'])) {
            return redirect()->back()->withErrors(['name' => 'Name can only contain letters and spaces.']);
        }
        // Check if the email is valid
        if(!filter_var($validatedData['email'], FILTER_VALIDATE_EMAIL)) {
            return redirect()->back()->withErrors(['email' => 'Invalid email format.']);
        }
