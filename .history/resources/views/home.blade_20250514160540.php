                ...
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <span class="dropdown-item-text text-muted"><i class="fas fa-envelope me-1"></i> {{ Auth::user()->email }}</span>
                            </li>
                            <li>
                    ...
                    <div class="card-body">
                        <form action="{{ route('posts.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" placeholder="Enter post title" required>
                                @error('title')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('body') is-invalid @enderror" id="body" name="body" rows="5" placeholder="What's on your mind?" required></textarea>
                                @error('body')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
            ...
            <div class="col-lg-8">
                ...
                <ul class="nav nav-tabs mb-4" id="postsTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="my-posts-tab" data-bs-toggle="tab" data-bs-target="#my-posts" type="button" role="tab" aria-controls="my-posts" aria-selected="true">
                            <i class="fas fa-user-edit me-2"></i>My Posts
                        </button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="all-posts-tab" data-bs-toggle="tab" data-bs-target="#all-posts" type="button" role="tab" aria-controls="all-posts" aria-selected="false">
                            ...
                            <div class="card shadow post-card">
                                <div class="card-body">
                                    <div class="post-header mb-3">
                                        <h3 class="h5 mb-0">{{ is_object($post) ? $post->title : 'Untitled Post' }}</h3>
                        ...
                        @else
                            ...
                            <div class="card shadow post-card">
                                <div class="card-body">
                                    <div class="post-header mb-3">
                                        <div>
                                            <h3 class="h5 mb-0 d-inline">{{ is_object($post) && property_exists($post, 'title') ? $post->title : 'Untitled Post' }}</h3>
                                            <span class="author-badge {{ Auth::id() == $post->user_id ? 'you' : '' }}">
                                                <i class="fas {{ Auth::id() == $post->user_id ? 'fa-user-circle' : 'fa-user' }} me-1"></i>
                                                {{ $post->user->name }} {{ Auth::id() == $post->user_id ? '(You)' : '' }}
                                            </span>
                                        </div>
                                        <small class="text-muted">{{ $post->created_at->diffForHumans() }}</small>
                                    </div>
